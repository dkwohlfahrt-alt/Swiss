...
document.getElementById("clear-players").onclick = () => {
  if (!confirm("Clear all players and tournaments?")) return;
  players = [];
  tournaments = {};
  save();
  location.reload();
...
function renderPlayers() {
  ...
  players.forEach(p => {
    html += `<tr>
      <td><input type="checkbox" ${p.active ? "checked" : ""} onchange="toggleActive(${p.id})"></td>
      <td>${p.name}</td>
...
document.getElementById("start-division").onclick = () => {
  const selectedAges = Array.from(document.getElementById("division-select").selectedOptions)
                            .map(opt => opt.value);
  if (selectedAges.length === 0) return alert("Select at least one age group");

  const divisionPlayers = players
    .filter(p => selectedAges.includes(p.age) && p.active)
    .map(p => ({ ...p, points: 0, opponents: [] }));

  if (divisionPlayers.length < 2) return alert("Need at least 2 active players.");

  const divName = selectedAges.join("+");
  tournaments[divName] = { round: 1, players: divisionPlayers, rounds: [], archive: [] };
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    for (let j = i + 1; j < sorted.length; j++) {
      ...
      if (!sorted[i].opponents.includes(sorted[j].id)) {
        pairings.push({ white: sorted[i], black: sorted[j], result: null });
...
function submitResult(div, boardIndex, result) {
  ...
  else { white.points += 0.5; black.points += 0.5; }
...
function isRoundComplete(div) {
  ...
  return t.rounds[t.rounds.length - 1].every(g => g.result !== null);
}
...
function renderDivision(div, container) {
  ...
  currentRound.forEach((game, i) => {
    html += `<tr>
      <td>${i+1}</td>
      <td style="color:${colorForAge(game.white.age)}">${game.white.name} (${game.white.age.toUpperCase()})</td>
      <td style="color:${colorForAge(game.black.age)}">${game.black.name} (${game.black.age.toUpperCase()})</td>
      <td>${game.result === null 
        ...
        : formatResult(game.result)}</td>
    </tr>`;
  });
  html += "</table>";

  const pending = currentRound.filter(g => g.result === null).length;
  if (pending > 0) html += `<p style="color:red;">${pending} game(s) pending</p>`;

  html += "<h4>Standings</h4>";
  html += "<table><tr><th>Name</th><th>Age</th><th>Pts</th><th>Rating</th></tr>";
  t.players.sort((a,b)=>b.points-a.points).forEach(p => {
    html += `<tr>
      <td style="color:${colorForAge(p.age)}">${p.name}</td>
      <td>${p.age.toUpperCase()}</td>
      <td>${p.points}</td>
...
function colorForAge(age) {
  return age === 'u9' ? 'blue' : age === 'u11' ? 'green' : 'orange';
}
